...
static int data_Callback(void* context, void *buf, int size, void* handle) {
	return st_inputFrameListTail((tp_playlistinst_t *) handle, (char *) buf,
			size);
}
...
static void mp4Record_cb(void* fileHandle, int track_video, int track_audio,
	...
	if (SANTACHI_I_FRAME_TYPE == keyframe) {
		...
		if (0 == isSetKeyFrame) {
			tp_mp4_write_videoFrame(fileHandle, track_video, keyFrameBuf,
					keyFrameLen, 1);
			isSetKeyFrame = 1;
		} else
		...
		while (pkt_cnt-- > 0) {
			if (RetFalse
					== tp_mp4_write_audioFrame(fileHandle, track_audio,
...
jint Java_com_topvs_platform_tools_LibAPI_InitLibInstance(JNIEnv* env, jclass jobject) {
...
jint Java_com_topvs_platform_tools_LibAPI_DeleteLibInstance(JNIEnv* env, jclass jobject) {
	int ret = -1;
...
jint Java_com_topvs_platform_tools_LibAPI_RequestLogout(JNIEnv* env, jclass jobject) {
	return RequestLogout(pLibInstParam);
}

jint Java_com_topvs_platform_tools_LibAPI_RequestLogin(JNIEnv* env,
		jclass jobject, jstring ip_addr, jint port, jstring user_name,
		jstring userpasword) {
...
//get ack cmd code
jint Java_com_topvs_platform_tools_LibAPI_GetCheckCode(JNIEnv* env, jclass jobject, jstring ip_addr, jint port, jstring user_name, jstring userpasword) {
	jboolean iscopy = JNI_TRUE;
	const char *addr = (*env)->GetStringUTFChars(env, ip_addr, &iscopy);
	const char *name = (*env)->GetStringUTFChars(env, user_name, &iscopy);
	const char *password = (*env)->GetStringUTFChars(env, userpasword, &iscopy);
...
//login with ack code
jint Java_com_topvs_platform_tools_LibAPI_RequestLoginEx(JNIEnv* env,
		jclass jobject, jstring ip_addr, jint port, jstring user_name,jstring userpasword, jstring checkcode) {
...
static void InsertNodeToList(DEVICE_NODE * srcList, int srcIndex, AREA_LIST *areaList, int areaIndex, int nodeType) {
	areaList[areaIndex].childNum = 0;
	if (nodeType == 0) {
		sprintf(areaList[areaIndex].AreaID, "%s", srcList[srcIndex].DomainID);
		int ret = sprintf(areaList[areaIndex].AreaName, "%s",
				srcList[srcIndex].DomainName);
		if (ret <= 0)
			sprintf(areaList[areaIndex].AreaName, "%s",
					srcList[srcIndex].AreaName);
	} else if (nodeType == 1) {
...
static void GetGUInfosList(const char* queryID, int iType){
	//LinZh107
	...
	if (queryID == NULL && iType == 0) {
		...
		ret = GetDeviceList(pLibInstParam, queryID, iType, tmpList,
				AREA_GROUP_NUM, &iNodeCnt);
		if (ret < 0 || iNodeCnt <= 0) {
			if (NULL != tmpList) {
				free(tmpList);
				tmpList = NULL;
	...
	//indicated the second level , also is the area level
		...
		memcpy(&pDeviceList->nodearray[iDevNodeCount], tmpList,
				sizeof(DEVICE_NODE) * iNodeCnt);
...
//jobjectArray Java_com_topvs_platform_tools_LibAPI_GetDeviceList(JNIEnv* env, jclass jobj, jintArray pOutArray)
void Java_com_topvs_platform_tools_LibAPI_GetDeviceList(JNIEnv* env,
		jclass jobj, jintArray DevNumArray, jintArray CamStatusArray,
		jintArray SWiStatusArray, jintArray SWoStatusArray,
		jintArray AreaChildArray, jobjectArray ArrayAreaStr,
		jobjectArray ArrayGUID, jobjectArray ArrayGUName) {
	jstring str = NULL;
	...
	pDeviceList->nodearray = (DEVICE_NODE*) malloc(
			sizeof(DEVICE_NODE) * MAX_NODE_NUM);
...
jint Java_com_topvs_platform_tools_LibAPI_StartPlay(JNIEnv* env, jclass jobject,
		jint iNodeIndex, jint viindex) {
	...
	memcpy(&iclient.guInfo, &pDeviceList->nodearray[iNodeIndex].guInfo,
			sizeof(GUINFO));
	...
	if (g_frameBufferOut[viindex] == NULL) {
		g_frameBufferOut[viindex] = (void*) malloc(
				MAX_FRAMESIZE * sizeof(char));
...
jint Java_com_topvs_platform_tools_LibAPI_StopPlay(JNIEnv* env, jclass jobject,
		jint viindex) {
...
jint Java_com_topvs_platform_tools_LibAPI_GetFrameNum(JNIEnv* env,
		jclass jobject, jint viindex) {
	VideoInstance_t *pVInst = (VideoInstance_t *) g_videoInstArray[viindex];
	return getFrameNum(pVInst->playinst, VIDEO_FRAME);
}

//Data: 2014-5-8 modify by LinZh107
jint Java_com_topvs_platform_tools_LibAPI_GetVideoFrame(JNIEnv* env,
		jclass jobj, jintArray pOutArray, jobject bitmap, jint viindex) {
	int iRet = -1;
	VideoInstance_t *pVInst = (VideoInstance_t *) g_videoInstArray[viindex];
	if (!pVInst)
		return iRet;
	...
	Hi3510_frame_t *pVideoFrame = st_getFrameListHead(pVInst->playinst,
			VIDEO_FRAME);
	...
	int ret = decode_H264_frame(pVideoFrame->frame_rate,
			pVideoFrame->frame_size, pVideoFrame->data, outArray,
			g_frameBufferOut[viindex], viindex);
	if (ret != 0) {
	...
	while (nlist < bmpinfo.height) {
		...
		frameLine = (uint8_t *) g_frameBufferOut[viindex]
				+ (nlist * outArray[0] * 2);
	...
	error: st_removeFrameListHead(pVInst->playinst, VIDEO_FRAME);
	pthread_mutex_unlock(&pVInst->dataList_mutex);
...
jint Java_com_topvs_platform_tools_LibAPI_GetAudioParam(JNIEnv *env,
		jobject obj, jintArray pOutArray, jint viindex) {
	VideoInstance_t *pVInst = (VideoInstance_t *) g_videoInstArray[viindex];
	if (!pVInst)
		return -1;

	Hi3510_frame_t *pAudioFrame = st_getFrameListHead(pVInst->playinst,
			AUDIO_FRAME);
...
jshortArray Java_com_topvs_platform_tools_LibAPI_GetAudioFrame(JNIEnv *env,
		jobject obj, jint frame_cnt, jint viindex) {
	VideoInstance_t *pVInst = (VideoInstance_t *) g_videoInstArray[viindex];
	if (!pVInst)
		return NULL;
...
jint Java_com_topvs_platform_tools_LibAPI_StartRecord(JNIEnv* env,
		jclass jobject, jstring filePath, jint viindex) {
...
jint Java_com_topvs_platform_tools_LibAPI_StopRecord(JNIEnv* env,
		jclass jobject, jint viindex) {
...
#if 0
jint Java_com_topvs_platform_tools_LibAPI_StartPlayRecord(JNIEnv* env, jclass jobj,
...
jint Java_com_topvs_platform_tools_LibAPI_GetRecordFrame(JNIEnv* env, jclass jobj,
...
jint Java_com_topvs_platform_tools_LibAPI_StopPlayRecord(JNIEnv* env, jclass jobj, jint viindex)
{
	VideoInstance_t *pVInst = (VideoInstance_t *)g_videoInstArray[viindex];
	if(!pVInst)
	return -1;
...
jint Java_com_topvs_platform_tools_LibAPI_OpenRecordFile(JNIEnv* env,
		jclass jobj, jstring filePath, jint viindex) {
	//LOGI("startPlayRecord start");
	jboolean iscopy = JNI_TRUE;
	const char *path = (*env)->GetStringUTFChars(env, filePath, &iscopy);
...
jint Java_com_topvs_platform_tools_LibAPI_GetRecordVideoFrame(JNIEnv* env,
		jclass jobj, jint params, jintArray pOutArray, jobject bitmap,
		jint viindex) {
	...
	//begin to decode the frame
	iRet = decode_H264_frame(25/*frame_rate*/, nSize, trparam->vBuffer,
			outArray, g_frameBufferOut[viindex], viindex);
	...
	ret_out: (*env)->ReleaseIntArrayElements(env, pOutArray, outArray, 0);
	return iRet;
}

jshortArray Java_com_topvs_platform_tools_LibAPI_GetRecordAudioFrame(
		JNIEnv* env, jclass jobj, jint params, jint frame_cnt, jint viindex) {
	mp4TracksParam_t *trparam = (mp4TracksParam_t *) params;
	int iRet;
...
jint Java_com_topvs_platform_tools_LibAPI_CloseRecordFile(JNIEnv* env,
		jclass jobj, jint params, jint viindex) {
	release_decoder(viindex);
...
jint Java_com_topvs_platform_tools_LibAPI_DomeControl(JNIEnv* env,
		jclass jobject, jint iNodeIndex, jint state, jint iSpeed,
		jstring strcmd) {
	...
	return DomeControl(pLibInstParam, *pGuInfo, ptzCtl.msgtype, ptzCtl.speed,
			ptzCtl.cmd, ptzCtl.param);
}

jint Java_com_topvs_platform_tools_LibAPI_GetPuImageDisplayPara(JNIEnv* env,
		jclass jobject, jint iNodeIndex, jintArray pOutArray) {
	//GUINFO *pGuInfo = g_GuInfoArray[iNodeIndex];
	GUINFO *pGuInfo = &pDeviceList->nodearray[iNodeIndex].guInfo;
	jint *intArray = (*env)->GetIntArrayElements(env, pOutArray, 0);

	int ret = GetPuImageDisplayPara(pLibInstParam, *pGuInfo, &intArray[0],
...
jint Java_com_topvs_platform_tools_LibAPI_SetPuImageDisplayPara(JNIEnv* env,
		jclass jobject, jint iNodeIndex, jintArray pOutArray) {
//	GUINFO *pGuInfo = g_GuInfoArray[iNodeIndex];
	GUINFO *pGuInfo = &pDeviceList->nodearray[iNodeIndex].guInfo;
	jint *intArray = (*env)->GetIntArrayElements(env, pOutArray, 0);

	int ret = SetPuImageDisplayPara(pLibInstParam, *pGuInfo, intArray[0],
			intArray[1], intArray[2], intArray[3]);
...
jint Java_com_topvs_platform_tools_LibAPI_GetPuImageEncodePara(JNIEnv* env,
		jclass obj, jint iNodeIndex, jintArray pOutArray) {
	jint *intArray = (*env)->GetIntArrayElements(env, pOutArray, 0);
//	GUINFO *pGuInfo = g_GuInfoArray[iNodeIndex];
	GUINFO *pGuInfo = &pDeviceList->nodearray[iNodeIndex].guInfo;
...
jint Java_com_topvs_platform_tools_LibAPI_SetPuImageEncodePara(JNIEnv* env,
		jclass obj, jint iNodeIndex, jintArray pOutArray) {
	jint *intArray = (*env)->GetIntArrayElements(env, pOutArray, 0);
//	GUINFO *pGuInfo = g_GuInfoArray[iNodeIndex];
	GUINFO *pGuInfo = &pDeviceList->nodearray[iNodeIndex].guInfo;
...
jint Java_com_topvs_platform_tools_LibAPI_RemoteAlarmEnable(JNIEnv* env,
		jclass jobj, jint iNodeIndex, jint bEnable) {
	//GUINFO *pGuInfo = g_GuInfoArray[iNodeIndex];
	GUINFO *pGuInfo = &pDeviceList->nodearray[iNodeIndex].guInfo;
	int ret;
	if (bEnable > 0)
...
jint Java_com_topvs_platform_tools_LibAPI_GetdevAlarmInfo(JNIEnv* env,
		jclass jcls, jobject jobj_out) {
	...
	jfieldID out_alarmtime = (*env)->GetFieldID(env, AlarmInfoCls, "alarmtime",
			"I");
	jfieldID out_airtemp = (*env)->GetFieldID(env, AlarmInfoCls, "airtemp",
			"F");
	jfieldID out_airhumi = (*env)->GetFieldID(env, AlarmInfoCls, "airhumi",
			"F");
	jfieldID out_soiltemp = (*env)->GetFieldID(env, AlarmInfoCls, "soiltemp",
			"F");
	jfieldID out_soilhumi = (*env)->GetFieldID(env, AlarmInfoCls, "soilhumi",
			"F");
	jfieldID out_CO2density = (*env)->GetFieldID(env, AlarmInfoCls,
			"CO2density", "F");
	jfieldID out_illuminance = (*env)->GetFieldID(env, AlarmInfoCls,
			"illuminance", "F");

	jfieldID out_water_sat = (*env)->GetFieldID(env, AlarmInfoCls, "water_sat",
			"F"); //soil water saturation
	jfieldID out_daily_rain = (*env)->GetFieldID(env, AlarmInfoCls,
			"daily_rain", "F"); //daily rainfall
	jfieldID out_anion = (*env)->GetFieldID(env, AlarmInfoCls, "anion", "F");
	jfieldID out_pm25 = (*env)->GetFieldID(env, AlarmInfoCls, "pm25", "F");
	jfieldID out_wind_speed = (*env)->GetFieldID(env, AlarmInfoCls,
			"wind_speed", "F");
	...
	(*env)->SetFloatField(env, jobj_out, out_CO2density,
			netAlarminfo.CO2density);
	(*env)->SetFloatField(env, jobj_out, out_illuminance,
			netAlarminfo.illuminance);
	(*env)->SetFloatField(env, jobj_out, out_water_sat, netAlarminfo.water_sat);
	(*env)->SetFloatField(env, jobj_out, out_daily_rain,
			netAlarminfo.daily_rain);
	(*env)->SetFloatField(env, jobj_out, out_anion, netAlarminfo.anion);
	(*env)->SetFloatField(env, jobj_out, out_pm25, netAlarminfo.pm25);
	(*env)->SetFloatField(env, jobj_out, out_wind_speed,
			netAlarminfo.wind_speed);

	return 0;
}

jint Java_com_topvs_platform_tools_LibAPI_StartChart(JNIEnv* env, jclass jcls,
		jint iNodeIndex, jint viindex) {
	LOGI("in %s\n", __FUNCTION__);
	...
	memcpy(&client.guInfo, &pDeviceList->nodearray[iNodeIndex].guInfo,
			sizeof(GUINFO));
...
jint Java_com_topvs_platform_tools_LibAPI_SendVoiceData(JNIEnv* env,
		jclass jcls, jshortArray AudioArray, jint dataLen) {
	//LOGI("in %s\n",__FUNCTION__);
	...
	ret = SendVoiceData(pLibInstParam, sendbuf,
...
jint Java_com_topvs_platform_tools_LibAPI_StopChart(JNIEnv* env, jclass jcls) {
	LOGI("in %s", __FUNCTION__);
	return StopChart(pLibInstParam);
}

jint Java_com_topvs_platform_tools_LibAPI_IoControl(JNIEnv* env, jclass jcls,
...
jint Java_com_topvs_platform_tools_LibAPI_setDecoderParam(JNIEnv* env,
		jclass jcls, jint viindex, jint videoMaxWidth, jint decodRate) {
